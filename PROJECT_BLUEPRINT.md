...
9. [Прямые вебхуки Instagram и Telegram](#9-прямые-вебхуки-instagram-и-telegram)
10. [Личный кабинет блогера (Partner Portal)](#10-личный-кабинет-блогера-partner-portal)
11. [Переменные окружения](#11-переменные-окружения)
12. [Деплой (VPS + GitHub Actions)](#12-деплой-vps--github-actions)
13. [Настройка Resend (домен и DNS)](#13-настройка-resend-домен-и-dns)
14. [Настройка ЮKassa](#14-настройка-юkassa)
15. [Открытые задачи](#15-открытые-задачи)
16. [Частые ошибки и решения](#16-частые-ошибки-и-решения)
17. [Чеклист запуска нового проекта](#17-чеклист-запуска-нового-проекта)
...
project/
├── app/
│   ├── __init__.py          # Фабрика Flask-приложения (logging настроен здесь)
...
│       ├── admin_bloggers_analytics.html    # Аналитика с фильтрами по каналу и периоду
│       ├── admin_bloggers_templates.html    # Шаблоны писем (вкладки: партнёрка / разовая)
│       ├── admin_blogger_detail.html        # Карточка блогера + размещения + ROI
│       ├── admin_placements.html            # Все размещения (фикс-модель)
│       ├── admin_budget.html                # Бюджет по месяцам
│       ├── admin_telegram_setup.html        # Настройка Telegram-бота + проверка прокси
...
├── scripts/
│   └── setup_webhooks.py    # Устарел — используй /admin/telegram/setup
├── run.py                   # Точка входа
├── requirements.txt
...
| Веб-сервер | Gunicorn (2 воркера, `127.0.0.1:8000`) |
| Email | Resend (Python SDK) — напрямую, без Make.com |
| Оплата | ЮKassa (`yookassa`) |
| AI для CRM | Anthropic Claude Haiku — классификация ответов и черновики |
| Instagram | Instagram Messaging API (IGAA-токен, Graph API v23.0) |
| Telegram | Telegram Bot API (webhook) |
| Хостинг | VPS (Ubuntu), nginx + SSL, systemd |
| CI/CD | GitHub Actions → SCP → systemd restart |
...
CRM поддерживает две модели — переключатель есть в форме добавления и редактирования блогера:

| Модель | cooperation_model | Описание |
|--------|-------------------|----------|
| 🤝 Партнёрка | `partnership` | 30% с каждой продажи, UTM-ссылка |
| 💳 Разовая реклама | `fix` | Фиксированная оплата за размещение |
...
BLOGGER_STATUSES = [
    ...
    ('agreed',          'Сотрудничаем', '#6a1b9a', '#f3e5f5'),  # фиолетовый
    # Статусы фикс-модели:
...
### Новые поля в таблице bloggers (добавлены через ALTER TABLE)

```sql
cooperation_model TEXT DEFAULT 'partnership'  -- partnership / fix
audience_size     INTEGER                      -- охват аудитории
topic             TEXT DEFAULT ''              -- тематика канала
er_percent        REAL                         -- engagement rate
channel           TEXT DEFAULT 'email'         -- email / instagram / telegram
ig_username, ig_user_id, tg_username, tg_user_id  -- контакты
partner_token     TEXT                         -- постоянный токен ЛК
```

### Таблица placements (размещения, фикс-модель)
...
CREATE TABLE placements (
    ...
    placement_date TEXT,
    format TEXT DEFAULT 'post',      -- post/story_1_24/story_1_48/reels/native/video
    model_at_placement TEXT DEFAULT 'fix',
    cost INTEGER DEFAULT 0,          -- стоимость в копейках
    payment_status TEXT DEFAULT 'not_required',
    views INTEGER, clicks INTEGER, sales INTEGER,
...
- **CAC** = cost_kopecks / 100 / sales (стоимость привлечения)
- **ROI** = (profit / cost) × 100
- **Маржа** = 69000 коп (фикс) или 48300 коп (партнёрка) с продажи
...
| `/admin/bloggers/<id>` | GET | Карточка блогера + размещения |
| `/admin/bloggers/<id>/update` | POST | Редактировать (сохраняет cooperation_model, cost в placement) |
| `/admin/bloggers/<id>/delete` | POST | Удалить |
| `/admin/bloggers/<id>/send-email` | POST | Отправить письмо (param: type=first/second/fix_first/fix_materials) |
| `/admin/bloggers/send-bulk` | POST | Массовая рассылка |
| `/admin/bloggers/analytics` | GET | Аналитика с фильтрами: канал, дата от/до, быстрые периоды |
| `/admin/bloggers/templates` | GET | Шаблоны (вкладки: партнёрка / разовая реклама) |
| `/admin/bloggers/templates/save` | POST | Сохранить шаблон |
| `/admin/placements` | GET | Все размещения с фильтрами и CSV |
| `/admin/budget` | GET | Бюджет по месяцам |
| `/api/placements` | GET/POST | CRUD API размещений |
| `/api/placements/<id>` | GET/PATCH/DELETE | Конкретное размещение |
| `/api/budgets/current` | GET | Бюджет текущего месяца |
| `/admin/server-ip` | GET | Показывает исходящий IP сервера (для настройки прокси) |

---

---

## 9. Прямые вебхуки Instagram и Telegram

Make.com полностью убран. Все входящие сообщения принимаются напрямую Flask-приложением.

### Архитектура входящих сообщений

```
Instagram DM → POST /webhook/instagram → store_incoming_message()
                                                  ↓ фоновый поток
                                         process_incoming_message_async()
                                                  ↓
                                         Claude Haiku: positive / negative / question
                                                  ↓
                  positive → статус "interested" → автоотправка письма 2
                  negative → статус "declined" → уведомление на почту
                  question → статус "replied" → черновик ответа → уведомление на почту

Telegram → POST /webhook/telegram → (тот же pipeline)
```
...
| Маршрут | Описание |
|---------|----------|
| `GET /webhook/instagram` | Верификация Meta (challenge) |
| `POST /webhook/instagram` | Входящие DM, HMAC SHA256 подпись |
| `POST /webhook/telegram` | Входящие сообщения, секретный заголовок |
| `GET /webhook/instagram/health` | Проверка статуса |
| `GET /webhook/telegram/health` | Проверка статуса |

### Авто-определение блогера по PSID

Instagram передаёт только числовой PSID отправителя, без username. Алгоритм:

1. Поиск по сохранённому `ig_user_id` — если уже привязан, готово
2. Поиск по `ig_username` из payload (в новом API бывает редко)
3. В фоновом потоке: запрос к Graph API `GET /v23.0/{psid}?fields=username` (таймаут 20с)
   - Если успешно → сохраняет `ig_user_id`, обрабатывает сообщение
   - Если не удалось → уведомление на `team.verevery@gmail.com` с PSID и текстом

### Переменные окружения для вебхуков

```env
META_VERIFY_TOKEN=          # любая строка, вводится в Meta Developer Console
META_APP_SECRET=            # App Secret из Meta App Dashboard (для HMAC проверки)
INSTAGRAM_ACCESS_TOKEN=     # IGAA... токен (Instagram User Access Token)
INSTAGRAM_USER_ID=          # числовой ID бизнес-аккаунта Instagram
TELEGRAM_BOT_TOKEN=         # токен от @BotFather
TELEGRAM_WEBHOOK_SECRET=    # любая строка, задаётся при регистрации webhook
```

### Настройка Instagram webhook в Meta Developer Console

1. Зайти в [developers.facebook.com](https://developers.facebook.com) → твоё приложение
2. **Webhooks** → Instagram → Edit → добавить:
   - Callback URL: `https://verevery.ru/webhook/instagram`
   - Verify Token: значение `META_VERIFY_TOKEN` из `.env`
3. Подписаться на поле `messages`

### Регистрация Telegram webhook

Через браузер: `/admin/telegram/setup` → кнопка «Зарегистрировать вебхук».

Или вручную:
```bash
curl "https://api.telegram.org/bot{TOKEN}/setWebhook" \
  -d "url=https://verevery.ru/webhook/telegram" \
  -d "secret_token={TELEGRAM_WEBHOOK_SECRET}"
```

### ⚠️ Ограничение: российский VPS блокирует Telegram и Instagram

**Telegram** (`api.telegram.org`) и **Instagram** (Meta серверы) недоступны с российского VPS — провайдеры блокируют исходящие соединения к этим IP.

**Решение:** прокси-сервер вне России, переменная `PROXY_URL`.

```env
PROXY_URL=http://login:pass@host:port   # HTTP CONNECT прокси
# или
PROXY_URL=socks5://login:pass@host:port  # SOCKS5 (требует PySocks)
```

**Требования к прокси:**
- Residential IP (не датацентр) — Telegram блокирует датацентровые диапазоны
- Порт 443 или 80 (порт 8000 блокируется российскими провайдерами)
- Рекомендуется: webshare.io (бесплатно 10 прокси/мес)
- Не подходит: proxy6.net (датацентр, порт 8000)

Код уже поддерживает прокси — все вызовы `requests` к Telegram и Instagram автоматически используют `_PROXIES` при наличии `PROXY_URL`.

---

## 10. Личный кабинет блогера (Partner Portal)

Блогер получает доступ к личному кабинету через магическую ссылку — без пароля.
...
Файл `/opt/verevery/.env` на сервере. Автоматически пополняется при деплое через GitHub Actions.
...
# Resend (email — напрямую, без Make)
RESEND_API_KEY=re_abc123...

# Anthropic (классификация ответов блогеров + черновики)
ANTHROPIC_API_KEY=sk-ant-abc123...

# Уведомления и reply-to
ADMIN_NOTIFY_EMAIL=team.verevery@gmail.com
REPLY_TO_EMAIL=team.verevery@gmail.com

# Безопасность webhook ЮKassa
WEBHOOK_SECRET=случайная-строка

# Instagram (прямой API)
META_VERIFY_TOKEN=        # для верификации webhook в Meta Console
META_APP_SECRET=          # для проверки HMAC подписи входящих
INSTAGRAM_ACCESS_TOKEN=   # IGAA... токен
INSTAGRAM_USER_ID=        # числовой ID бизнес-аккаунта

# Telegram (прямой API)
TELEGRAM_BOT_TOKEN=       # токен от @BotFather
TELEGRAM_WEBHOOK_SECRET=  # секрет для заголовка X-Telegram-Bot-Api-Secret-Token

# Прокси (для Telegram и Instagram — обязателен на российском VPS)
PROXY_URL=                # формат: http://login:pass@host:port или socks5://...
```

### Важно: прокси на российском VPS

Российские хостинги блокируют исходящие соединения к Telegram API (`api.telegram.org`) и Meta серверам. Без прокси отправка сообщений и входящие API-запросы работать не будут.

**Требования к прокси:**
- Порт **443 или 80** (порт 8000 обычно заблокирован провайдером)
- Residential (не datacenter) — Telegram блокирует датацентровые IP
- Рекомендуется: **webshare.io** (есть бесплатный тариф на 10 прокси)
- НЕ подходит: proxy6.net (datacenter IP, заблокированы Telegram; порт 8000)

Код уже готов — `_PROXIES` применяется во всех запросах к Telegram и Instagram автоматически.

---

## 12. Деплой (VPS + GitHub Actions)
...
yookassa==3.1.0
anthropic==0.40.0
PySocks==1.7.1       # для SOCKS5 прокси
```
...
## 13. Настройка Resend (домен и DNS)
...
|--------|--------|---------|
| Telegram и Instagram не работают с VPS | ⏳ В процессе | Нужен residential прокси на порту 443/80. Куплен proxy6.net — **не подошёл** (datacenter IP, порт 8000 заблокирован). Следующий шаг: взять прокси на **webshare.io** (есть бесплатный тариф), добавить в `PROXY_URL`. Код готов. |

**Что уже сделано для прокси:**
- Добавлена переменная `PROXY_URL` в `.env` и deploy.yml
- Установлен `PySocks` в requirements.txt (для SOCKS5)
- Все вызовы `requests` к Telegram и Instagram передают `proxies=_PROXIES`
- Добавлена страница `/admin/telegram/setup` — проверка бота и регистрация вебхука
- Добавлен `/admin/server-ip` — показывает исходящий IP сервера

**Что осталось:**
1. Зарегистрироваться на webshare.io → получить прокси на порту 80
2. На сервере в `/opt/verevery/.env` прописать `PROXY_URL=http://login:pass@host:port`
3. `systemctl restart verevery`
4. Зайти в `/admin/telegram/setup` → «Проверить» → «Зарегистрировать вебхук»
5. Написать боту `@Verevery_crm_bot` — должно появиться в CRM

### 🟡 Важно

| Задача | Описание |
|--------|----------|
| Telegram бот `@Verevery_crm_bot` | Создан, токен настроен. Вебхук зарегистрирован на `https://verevery.ru/webhook/telegram`. Не работает из-за отсутствия рабочего прокси. |
| Instagram DM | Та же причина — Meta серверы недоступны с VPS без прокси. |

### 🟢 Улучшения (по желанию)

| Задача | Описание |
|--------|----------|
| Ответы блогеров на email | Читаются вручную в Gmail. Для автоматики — настроить Resend Inbound |
| Уведомление если нет ответа 7 дней | Добавить фоновую задачу (APScheduler) |
| Конвертация с фикс в партнёрку | Баннер на странице блогера уже есть, нужно протестировать сценарий |
...
## 17. Чеклист запуска нового проекта
