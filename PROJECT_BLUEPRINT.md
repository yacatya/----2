...
### Ограничение: исходящие сообщения в Instagram

⚠️ **Отправка DM через Instagram API не работает с российского VPS** — провайдеры блокируют исходящие TLS-соединения к серверам Meta. Принимать входящие при этом можно (Instagram инициирует соединение сам).

Для отправки DM нужен прокси-сервер за пределами России (см. [открытые задачи](#15-открытые-задачи)).
...
|--------|---------|---------|
| Отправка DM в Instagram не работает | Российский VPS блокирует TLS к серверам Meta | Купить HTTP/SOCKS5 прокси (~150 ₽/мес) вне России, добавить `META_PROXY=http://login:pass@host:port` в `.env`, код уже готов к этой переменной |
| Авто-определение блогера по PSID не работает | Та же причина — Graph API недоступен с VPS | То же решение (прокси), либо добавить PSID вручную один раз в карточку блогера |
...
| Задача | Описание |
|--------|----------|
| Ответы блогеров на email | Сейчас обрабатываются вручную (читаешь Gmail). Для автоматики — настроить Resend Inbound на отдельный поддомен |
| Рассылка третьего письма | Маршрут `/admin/bloggers/<id>/send-email` с `type=third` уже есть, нужно добавить кнопку в шаблон CRM |
| Уведомление если нет ответа 7 дней | Добавить фоновую задачу (cron или APScheduler) |
| Аналитика по блогерам | Страница `/admin/bloggers/analytics` есть, можно расширить воронку |
